...
const Register = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            toast.error(err.response?.data?.msg || 'Erro no cadastro');
        }
    };
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-[#4a0404] p-6 relative overflow-hidden">
            ...
            <div className="absolute top-0 right-0 w-96 h-96 bg-[#f5f5dc]/5 rounded-full blur-[120px] -mr-48 -mt-48" />
            ...
            >
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    <div>
                        <label className="block text-sm font-bold text-[#f5f5dc]/80 mb-2 ml-1">Nome Completo</label>
                        <div className="relative">
                            <User className="absolute left-4 top-1/2 -translate-y-1/2 text-[#f5f5dc]/40" size={20} />
                            <input
                                type="text"
                                required
                    ...
                    <button
                        type="submit"
                        className="w-full bg-[#f5f5dc] hover:bg-[#e8e8c1] text-[#4a0404] font-black py-4 rounded-2xl shadow-xl shadow-black/30 transition-all active:scale-[0.97] flex items-center justify-center gap-3 group text-lg mt-4"
                    >
                        CRIAR ACESSO
                ...
                <p className="mt-8 text-center text-[#d1d1d1] font-medium">
                    Já possui acesso?{' '}
                    <Link to="/login" className="text-[#f5f5dc] font-black hover:text-white transition-colors underline decoration-[#f5f5dc]/30 underline-offset-4">
                        Faça login
                    </Link>
                </p>
            </motion.div>
